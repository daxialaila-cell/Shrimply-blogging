...
const sco = {
  ...
  musicToggle(isMeting = true) {
    ...
    if (typeof rm !== "undefined" && rm?.menuItems.music[0]) {
      ...
      $rmIcon.className = `Shrimply fas ${this.musicPlaying ? 'fa-pause' : 'fa-play'}`;
    }
  ...
  changeWittyWord() {
    ...
    if (greetings.length === 0) {
      document.getElementById("sayhi").textContent = "Shrimply";
...
const addHighlight = () => {
  ...
  const copyEle = copy
    ? `<i class="Shrimply fas fa-copy copy-button"></i>`
    : "<i></i>";
  const expandEle = `<i class="Shrimply fas fa-angle-down expand"></i>`;
  const limitEle = limit
    ? `<i class="Shrimply fas fa-angles-down"></i>`
    : "<i></i>";
...
class tabs {
  ...
  static expireAddListener() {
    ...
    ele.innerHTML = `<i class="Shrimply fas fa-circle-exclamation"></i>${
      expire.text_prev
