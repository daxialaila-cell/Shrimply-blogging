...
hexo.extend.helper.register('export_config', function () {
  ...
  return `<script id="fluid-configs">
    var Shrimply = window.Shrimply || {};
    Shrimply.ctx = Object.assign({}, Shrimply.ctx)
    var CONFIG = ${JSON.stringify(exportConfig)};

    if (CONFIG.web_analytics.follow_dnt) {
      var dntVal = navigator.doNotTrack || window.doNotTrack || navigator.msDoNotTrack;
      Shrimply.ctx.dnt = dntVal && (dntVal.startsWith('1') || dntVal.startsWith('yes') || dntVal.startsWith('on'));
    }
  </script>`;
});
