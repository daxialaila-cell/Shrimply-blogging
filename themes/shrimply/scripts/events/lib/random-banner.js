...
module.exports = (hexo) => {
  ...
  if (!fs.existsSync(randomDir)) {
    hexo.log.warn(`[Shrimply] Random banner directory not found: ${randomDir}`);
    return;
  }
  ...
  if (files.length === 0) {
    hexo.log.warn('[Shrimply] Random banner directory is empty.');
    return;
  }

  themeConfig.banner_img_list = files.map((file) => `/img/random/${file}`);
  hexo.log.debug(`[Shrimply] Random banner images loaded: ${themeConfig.banner_img_list.length}`);
};
