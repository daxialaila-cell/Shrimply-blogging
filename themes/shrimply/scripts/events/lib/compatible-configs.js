...
module.exports = (hexo) => {
  ...
  if (hexo.theme.config.highlight && !hexo.theme.config.code) {
    if (isZh) {
      hexo.log.warn('[Shrimply] 检测到弃用的配置项: "highlight" 已被修改为 "code:highlight"，请根据新版本更新');
    } else {
      hexo.log.warn('[Shrimply] Deprecated config detected: "highlight" has been modified to "code:highlight", please update according to the release.');
    }
  ...
  if (parseInt(hexo.version[0], 10) < 5) {
    if (isZh) {
      hexo.log.warn('[Shrimply] 检测到 Hexo 版本低于 5.0.0，部分功能可能会受影响');
    } else {
      hexo.log.warn('[Shrimply] Hexo version < 5.0.0 detected, some features may not be available.');
    }
  ...
  if (hexo.theme.config.index.post_default_img) {
    if (isZh) {
      hexo.log.warn('[Shrimply] 检测到弃用的配置项: "index:post_default_img" 已被修改为 "post:default_index_img"，请根据新版本更新');
    } else {
      hexo.log.warn('[Shrimply] Deprecated config detected: "index:post_default_img" has been modified to "post:default_index_img", please update according to the release.');
    }
  ...
  if (hexo.theme.config.banner_parallax) {
    if (isZh) {
      hexo.log.warn('[Shrimply] 检测到弃用的配置项: "banner_parallax" 已被修改为 "banner:parallax"，请根据新版本更新');
    } else {
      hexo.log.warn('[Shrimply] Deprecated config detected: "banner_parallax" has been modified to "banner:parallax", please update according to the release.');
    }
