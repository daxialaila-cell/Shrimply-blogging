...
module.exports = (hexo) => {
  ...
  if (hexo.locals.get instanceof Function) {
    ...
    if (data && isNotEmptyObject(data.fluid_config)) {
      ...
      if (isZh) {
        hexo.log.warn('[Shrimply] 推荐你使用覆盖配置功能: https://hexo.daxialaila-cell.com/docs/guide/#%E8%A6%86%E7%9B%96%E9%85%8D%E7%BD%AE');
      } else {
        hexo.log.warn('[Shrimply] It is recommended that you use override configuration: https://hexo.daxialaila-cell.com/docs/en/guide/#override-configuration');
      }
    }
    ...
    if (isNotEmptyObject(langConfigMap)) {
      ...
      if (isZh) {
        hexo.log.debug('[Shrimply] 读取 source/_data/languages/*.yml 文件覆盖语言配置');
      } else {
        hexo.log.debug('[Shrimply] Merge language config from source/_data/languages/*.yml');
      }
    }
  }
  ...
  if (isNotEmptyObject(hexo.config.theme_config)) {
    ...
    if (isZh) {
      hexo.log.debug('[Shrimply] 读取 _config.yml 中 theme_config 配置项覆盖主题配置');
    } else {
      hexo.log.debug('[Shrimply] Merge theme config from theme_config in _config.yml');
    }
  }
  ...
  if (isNotEmptyObject(dataStaticConfig)) {
    ...
    if (isZh) {
      hexo.log.debug('[Shrimply] 读取 source/_data/fluid_static_prefix.yml 文件覆盖主题配置');
    } else {
      hexo.log.debug('[Shrimply] Merge theme config from source/_data/fluid_static_prefix.yml');
    }
  }
  ...
  if (isNotEmptyObject(dataConfig)) {
    ...
    if (isZh) {
      hexo.log.debug('[Shrimply] 读取 source/_data/fluid_config.yml 文件覆盖主题配置');
    } else {
      hexo.log.debug('[Shrimply] Merge theme config from source/_data/fluid_config.yml');
    }
  }

  hexo.log.debug('[Shrimply] Output theme config:\n', JSON.stringify(hexo.theme.config, undefined, 2));
};
